...
function formatContactDate(val?: string | null): string {
  if (!val) return "Tarih belirtilmedi";
  try {
    const raw = String(val).trim();
    const isoString = raw.includes(" ") && !raw.includes("T") ? raw.replace(" ", "T") : raw;
    const d = new Date(isoString);
    if (isNaN(d.getTime())) {
      const fallback = new Date(raw);
      if (isNaN(fallback.getTime())) return "Bugün";
      return fallback.toLocaleDateString("tr-TR", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
    }
            ...
            <div className="admin-card-list">
              {filteredContacts.length ? (
                filteredContacts.map((item) => {
                  ...
                  const formattedPhone = cleanPhone.startsWith("0") ? `9${cleanPhone}` : cleanPhone.startsWith("90") ? cleanPhone : `90${cleanPhone}`;
                  const formattedDate = formatContactDate(item.createdAt || (item as any).created_at);
                  const initialLetter = (item.name || "M").trim().charAt(0).toUpperCase();

                  return (
                    <article
                      key={item.id}
                      className={`admin-contact-card ${item.status === "new" ? "is-new-message" : ""}`}
                    >
                      <div className="admin-contact-card-top">
                        <div className="admin-contact-badges">
                          <span className="admin-contact-subject-badge">{item.subject || "Genel İletişim Talebi"}</span>
                          <span className={`admin-contact-status-pill status-${item.status}`}>
                            <span className="status-indicator-dot" />
                            {item.status === "new" ? "Yeni Mesaj" : item.status === "read" ? "Okundu" : "Sonuçlandı"}
                          </span>
                        </div>
                        <span className="admin-contact-date-badge">
                      ...
                      <div className="admin-contact-sender-row">
                        <div className="admin-contact-avatar">{initialLetter}</div>
                        <div className="admin-contact-details">
                          <h3 className="admin-contact-name">{item.name}</h3>
                          <div className="admin-contact-meta-links">
                            <a href={`mailto:${item.email}`} className="admin-contact-meta-chip">
                              <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2">
                            ...
                            {item.phone ? (
                              <a href={`tel:${item.phone}`} className="admin-contact-meta-chip">
                                <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2">
                      ...
                      <div className="admin-contact-footer-actions">
                        <div className="admin-contact-quick-replies">
                          {cleanPhone ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="admin-contact-action-btn btn-whatsapp"
                            >
                        ...
                        <form onSubmit={(event) => updateContact(event, item.id)} className="admin-contact-status-form">
                          <div className="admin-contact-select-wrap">
                            <select name="status" defaultValue={item.status} className="admin-contact-status-select">
                              <option value="new">Yeni</option>
                              <option value="read">Okundu</option>
                              <option value="resolved">Sonuçlandı</option>
                            </select>
                            <svg className="admin-select-chevron" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2">
                              <polyline points="6 9 12 15 18 9" />
                            </svg>
                          </div>
                          <button type="submit" className="admin-contact-action-btn btn-save-status">
