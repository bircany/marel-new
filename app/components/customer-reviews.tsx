...
export function CustomerReviews({ products, reviews }: { products: Array<{ id: string; name: string }>; reviews: ReviewPayload[] }) {
  ...
  return <section className="customer-review-panel" id="yorumlar">
    ...
    <div className="customer-review-grid">
      <form className="customer-review-form" onSubmit={submit}>
        ...
        <label>Başlık<input name="title" maxLength={150} placeholder="Deneyiminizi özetleyin" /></label>
        <label>Yorumunuz<textarea name="body" maxLength={1000} rows={5} placeholder="Ürün, ölçü veya hizmet deneyiminizi anlatın" /></label>
      ...
      <div className="customer-review-history">{reviews.length ? reviews.map((review) => <article key={review.id}><div><span>{"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}</span><strong className={`review-state review-state-${review.status}`}>{statusNames[review.status] ?? review.status}</strong></div><h3>{review.title}</h3><small>{review.product?.name ?? "Genel Marel deneyimi"} · {new Date(review.created_at).toLocaleDateString("tr-TR")}</small><p>{review.comment}</p></article>) : <div className="customer-review-empty"><b>İlk yorumunuzu yazın</b><p>Gönderdiğiniz yorumlar ve yayın durumları burada görünür.</p></div>}</div>
    </div>
  </section>;
}
